...
public interface ProductDetail_Size_ColorRepository extends JpaRepository<ProductdetailColorSize ,Integer> {
    ...
    @Query(value = "Select p from ProductdetailColorSize p\n" +
            "where p.productDetail.id = :id and p.color.id = :color and p.size.id = :size")
    ProductdetailColorSize getByProductAndColorAndSize(@Param("id") Integer id,@Param("color") Integer color,@Param("size") Integer size);

    @Query(value = "Select pdcz.Id,pdcz.IdProductDetail,pdcz.IdColor,pdcz.IdSize,p.Code,p.Name,s.Name as NameSize,c.Name as NameColor,pdcz.Quantity ,pd.Price\n" +
            ...
            "where c.Status = 0 and p.Id = 1 order by c.CreateDate desc", nativeQuery = true)
